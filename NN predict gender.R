...
# Filter to only look at ADHD patients
data <- data %>% filter(ADHD_Outcome == 1)
...
# Convert Sex_F to numeric (0 and 1)
data$Sex_F <- as.numeric(data$Sex_F)

# Calculate class weights
class_counts <- table(data$Sex_F)
...
set.seed(123)
train_index <- createDataPartition(data$Sex_F, p = 0.8, list = FALSE)
train_data <- data[train_index, ]
test_data <- data[-train_index, ]

# Scale the numeric features - excluding Sex_F which is y
preprocess_params <- preProcess(train_data[, 
                                           -which(names(train_data) == "Sex_F")], 
